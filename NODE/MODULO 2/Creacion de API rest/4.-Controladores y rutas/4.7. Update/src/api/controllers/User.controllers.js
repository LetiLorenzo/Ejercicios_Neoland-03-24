...
/*
 * PASOS A SEGUIR: 
 * 1) Tener en cuenta qué cosas podemos y no podemos actualizar:
 ...
 * - confirmationCode
 * 
 * 2) todo lo que se pueda cambiar es lo que 
 * recibimos por el body: 
 * - name
 * - image
 *  
 * (para el resto de cosas que quisieramos cambiar 
 * hay que hacer un controlador específico para no romper 
 * la experiencia de usuario)
 * 
 * - (vamos a hacer que se pueda cambiar el género para ver qué podría suceder en el caso 
 * de que se diera. Para ello, hacemos un controlador específico para el género que para
 * poder hacer este cambio, tenemos que ser administrador)
 * 
 * 3) qué tenemos que comprobar primero con el usuario:
 * - tenemos un token ---> _id req.user
 * 
 * 4)Cuando tenemos el token y el usuario de id ya podemos hacer
 * la query para actualizar --> findByAndUpdate
 * 
 * 5) Por último hacemos el tets para ver si se ha actualizado o no.
 * 
 * 
 * Posibles ERRORES que puede haber:
 *  ---> que no recibamos el token, por lo cual, lo vamos a echar por el middleware de auth
 *  */
...
const update = async (req, res, next) => {
  ...
  try {
    ...
    try {
      ...
      //si ha habido un error, la imagen la borramos:
    } catch (error) {
      if (req.file) deleteImgCloudinary(catchImg);
      return res.status(404).json(error.message);
    }
    //con el catch general haríamos lo mismo:
  } catch (error) {
    if (req.file) deleteImgCloudinary(catchImg);
    return next(error);
  }
